...
def get_Quiz(correctness,prev_questions):
  ...
  api_key = 'api-key' #please add your API key here
  ...
  if number_selection < 550:
      ...
      for attemp in range(10):
        try:
            time.sleep(0.01)
            response_ack = requests.post(api_url, headers=headers, data=json.dumps(data1_send))
            time.sleep(1)
            break
      ...
      for attemp in range(10):
        try:
            while indicator_rep==1:
                time.sleep(0.01)
                data2_send['messages'][0]['content'] = data2_send['messages'][0]['content'] + repetition
                response = requests.post(api_url, headers=headers, data=json.dumps(data2_send))
                time.sleep(1)
                for ccount in range(0,len(prev_questions)):
                      body_question=prev_questions[ccount].split('\n')
                      if len(body_question[0])>0 and not(body_question[0]=='\n') and not(body_question[0]==' '):
                         #print(body_question[0],response.content.decode('utf8'),'response_interim')
                         string_question=response.json()['choices'][0]['message']['content'].lower().split('a)')
      ...
      for attemp in range(10):
        try:
            while indicator_rep==1:
                time.sleep(0.01)
                data1_send['messages'][0]['content'] = data1_send['messages'][0]['content'] + data2 + repetition
                response = requests.post(api_url, headers=headers, data=json.dumps(data1_send))
                time.sleep(1)
                for ccount in range(0,len(prev_questions)):
                      body_question=prev_questions[ccount].split('\n')
                      if len(body_question[0])>0 and not(body_question[0]=='\n') and not(body_question[0]==' '):
  ...
  print(response.json(), 'value_response')
